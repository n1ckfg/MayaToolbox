...
def paintSurface(target=None, brush="fire", bake=True, reducePolys=0.1, maxPolys=1000000):
    returns = []
    deleteCurves = []
    if not target:
        target = s()
...
def bakePaintEffects(target=None, reducePolys=0.1, maxPolys=1000000):
    if not target:
        target = s()
